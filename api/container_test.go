		...
		Context("when authenticated", func() {
			...
			Context("with no params", func() {
				...
				Context("when no errors are returned", func() {
					...
					BeforeEach(func() {
						...
						containerDB.ContainersReturns(fakeContainers, true, nil)
					})
				...
				Context("when no containers are found", func() {
					BeforeEach(func() {
						containerDB.ContainersReturns([]db.ContainerInfo{}, false, nil)
					})
				...
				Context("when there is an error", func() {
					...
					BeforeEach(func() {
						...
						containerDB.ContainersReturns([]db.ContainerInfo{}, false, expectedErr)
					})
			...
			Describe("querying with pipeline name", func() {
				...
				It("calls db.Containers with the queried pipeline name", func() {
					...
					Ω(containerDB.ContainersCallCount()).Should(Equal(1))
					Ω(containerDB.ContainersArgsForCall(0)).Should(Equal(expectedArgs))
				})
			})
				...
				Context("when the buildID can be parsed as an int", func() {
					...
					Context("when the buildID fails to be parsed as an int", func() {
						...
						It("does not lookup containers", func() {
							...
							Ω(containerDB.ContainersCallCount()).Should(Equal(0))
						})
					})
				})
			})
	...
	Describe("GET /api/v1/containers/:id", func() {
		...
		Context("when authenticated", func() {
			...
			Context("when the container is not found", func() {
				BeforeEach(func() {
					containerDB.GetContainerReturns(db.ContainerInfo{}, false, nil)
				})
			...
			Context("when the container is found", func() {
				...
				It("performs lookup by id", func() {
					...
					Ω(containerDB.GetContainerCallCount()).Should(Equal(1))
					Ω(containerDB.GetContainerArgsForCall(0)).Should(Equal(containerID))
				})
			...
			Context("when there is an error", func() {
				...
				BeforeEach(func() {
					expectedErr = errors.New("some error")
					containerDB.GetContainerReturns(db.ContainerInfo{}, false, expectedErr)
				})
	...
	Describe("POST /api/v1/containers/:id/hijack", func() {
		...
		Context("when authenticated", func() {
			...
			Context("and the worker client returns a container", func() {
				...
				BeforeEach(func() {
					fakeContainerInfo = db.ContainerInfo{}
					containerDB.GetContainerReturns(fakeContainerInfo, true, nil)
			...
			Context("when the db request fails", func() {
				BeforeEach(func() {
					...
					containerDB.GetContainerReturns(db.ContainerInfo{}, false, fakeErr)
				})
